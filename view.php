...
require_once(WB_PATH .'/modules/aggregator/include.php');

// Get current aggregator settings:
$settings_query = $database->query("SELECT * FROM ".TABLE_PREFIX."mod_aggregator WHERE section_id = '$section_id'");
...
if($num_of_pgs <= 6){
	for($count = 1; $count <= $num_of_pgs; $count++){
		$nav_page = spacer(1) . "<span>" . $count . "</span>\n";
		if($count != $pg){
	...
	if(($pg - 2) < 1){
		...
		$browser .= spacer(2) . "<span>" . $num_of_pgs . "</span>\n" . spacer(1) . "</a>\n";
	...
	//we have more than 6 pages, and we are navigating to a page somewhere in the middle:
		...
		$browser .= spacer(2) . "<span>" . ($pg - 1) . "</span>\n" . spacer(1) . "</a>\n";
		$browser .= spacer(1) . "<span>" . $pg . "</span>\n";
		$browser .= spacer(1) . "<a href=\"" . WB_URL . PAGES_DIRECTORY . $this_page['link'] . PAGE_EXTENSION . "?aggpage=" . ($pg + 1) . "\">\n";
		$browser .= spacer(2) . "<span>" . ($pg + 1) . "</span>\n" . spacer(1) . "</a>\n";
	...
	if($agg_settings['thumb_size'] && ($page_source != '')){
		...
		if(!$img_tag){ //if no image was found in the page
			...
			if(preg_match($regex, $img_tag, $match)) {
				$replacement = 'src="' . WB_URL . '/modules/aggregator/thumbnailer.php?img=';
